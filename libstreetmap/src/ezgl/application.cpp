...
void application::register_default_buttons_callbacks(ezgl::application *application)
{
  ...
  GObject *help_btn = application->get_object("helpbtn");
  g_signal_connect(help_btn, "clicked", G_CALLBACK(helpbtn_func), application);
  
  GObject *pool_button = application->get_object("drive_walk");
  g_signal_connect(pool_button, "clicked", G_CALLBACK(press_drive_walk), application);
...
void application::create_button_info(const char *button_text,
    int left,
    int top,
    int width,
    int height,
    button_callback_fn button_func)
{
  ...
  GtkGrid *in_grid = (GtkGrid *)get_object("InfoGrid");
...
void application::create_button_find(const char *button_text,
    int left,
    int top,
    int width,
    int height,
...
{
  // get the internal Gtk grid
  GtkGrid *in_grid = (GtkGrid *)get_object("findGrid");
